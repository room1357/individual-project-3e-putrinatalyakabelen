...
class ExpenseListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // Tambahkan filtering di sini
    // Mendapatkan hanya pengeluaran makanan
    final List<Expense> foodExpenses = expenses
        .where((expense) => expense.category.toLowerCase() == 'makanan')
        .toList();

    // Mendapatkan pengeluaran di atas 100.000
    final List<Expense> expensiveItems = expenses
        .where((expense) => expense.amount > 100000)
        .toList();

    // Mendapatkan hanya judul-judul pengeluaran
    final List<String> titles = expenses
        .map((expense) => expense.title)
        .toList();

    // Mendapatkan informasi ringkas
    final List<String> summaries = expenses
        ...
        children: [
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemCount: expenses.length, //jml total item
              itemBuilder: (context, index) { // Fungsi pembuat widget untuk setiap item
                final expense = expenses[index]; // Mengakses item berdasarkan index
                return Card(
